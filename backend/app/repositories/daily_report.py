...
class DailyReportRepository:
    ...
    async def get_by_id(self, report_id: uuid.UUID) -> Optional[DailyReport]:
        result = await self.db.execute(
            select(DailyReport).where(DailyReport.id == report_id, DailyReport.deleted_at.is_(None))
        )
    ...
    async def create(self, data: DailyReportCreate, created_by: uuid.UUID) -> DailyReport:
        report = DailyReport(**data.model_dump(), created_by=created_by, updated_by=created_by)
        self.db.add(report)
        await self.db.flush()
        await self.db.refresh(report)
        return report

    async def update(self, report: DailyReport, data: DailyReportUpdate,
                     updated_by: uuid.UUID) -> DailyReport:
