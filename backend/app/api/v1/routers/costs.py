...
from app.schemas.cost import (
    ...
    WorkHourCreate, WorkHourResponse
)

router = APIRouter(tags=["原価・工数管理"])


@router.post("/projects/{project_id}/cost-records",
             response_model=ApiResponse[CostRecordResponse], status_code=201)
async def create_cost_record(
    project_id: uuid.UUID, payload: CostRecordCreate,
...
@router.get("/projects/{project_id}/cost-records",
...
async def list_cost_records(
    ...
    ).order_by(CostRecord.record_date.desc()).offset((page-1)*per_page).limit(per_page)
    total_q = select(func.count()).select_from(CostRecord).where(
        CostRecord.project_id == project_id, CostRecord.deleted_at.is_(None)
    )
...
@router.get("/projects/{project_id}/cost-summary", response_model=ApiResponse[CostSummary])
async def get_cost_summary(
    ...
    return ApiResponse(data=CostSummary(
        ...
        by_category={r.category: {"budgeted": float(r.budgeted or 0),
                                   "actual": float(r.actual or 0)} for r in rows},
    ))


@router.post("/projects/{project_id}/work-hours",
             response_model=ApiResponse[WorkHourResponse], status_code=201)
async def create_work_hour(
    project_id: uuid.UUID, payload: WorkHourCreate,
    db: Annotated[AsyncSession, Depends(get_db)],
    current_user: Annotated[User, Depends(require_roles(
