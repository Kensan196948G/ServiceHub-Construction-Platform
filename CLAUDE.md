...
**プロジェクト期間（半年厳守 + フェーズ配分は CTO 自由）:**
- プロジェクト期間: **6 ヶ月（登録日から半年）絶対厳守**。本番リリース期限は **2026-10-03（登録日 2026-04-03 + 6 ヶ月）**
- 1 セッション最大作業時間: **5 時間厳守**（Linux Cron 月〜土、プロジェクト別スケジュール）
- 開発フェーズ配分（Monitor / Build / Verify / Improve）: 進捗に応じて **CTO 判断で自由に変更可**（半年期限の制約下で）
- 残日数による自動縮退:
  - 残 30 日以内: Improvement 縮退、Verify / リリース準備を優先
  - 残 14 日以内: 新機能開発禁止、バグ修正・安定化のみ
  - 残 7 日以内: リリース準備のみ（CHANGELOG / README / タグ付け）
  - 残 0 日以内: 即時停止 → CTO エスカレーション
...
- 残日数判定は GitHub Actions `release-deadline-check` workflow が日次実行 + state.json 反映で自動監視
