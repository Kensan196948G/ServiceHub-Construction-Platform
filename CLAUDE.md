...
## 0.0 プロジェクト期間・リリース期限（ユーザー厳守指示・最上位制約）

> 本節は本セクション全体・全ループ・全判断に優先する。以下の 5 項目を変更・緩和・延期してはならない。

| 項目 | 値 | 備考 |
|---|---|---|
| 🗓️ プロジェクト期間 | **6 ヶ月（半年）** | 登録日 `2026-04-03` 〜 リリース期限 `2026-10-03` |
| ⚙️ 実行方式 | **Linux Cron 固定** | 月〜土 自動起動、セッション制御は cron 側 |
| ⏱️ 1 セッション作業時間 | **5 時間厳守** | 延長不可。`state.json.execution.max_duration_minutes = 300` |
| 🔀 開発フェーズ配分 | **進捗に応じて CTO 判断で自由変更 OK** | Monitor/Development/Verify/Improvement の比率は動的 |
| 🚀 本番リリース | **半年後（2026-10-03）絶対厳守** | 延期・スコープ無限拡張禁止 |

**ユーザー指示原文（2026-04-23）**:
> プロジェクト期間は半年としてください。実行は Cron にて決まっています。作業時間は５時間、進み具合によって開発フェーズは自由に変更して OK です。但し半年後の本番リリースは守ってください。

### 残日数による自動縮退（Monitor フェーズで毎回判定）
...
| 残 0 日超過 | **Security blocker 扱いで全作業停止**、CTO エスカレーション |

残日数の機械可読ソースは `state.json.project.registered_at` / `state.json.project.release_deadline`。
