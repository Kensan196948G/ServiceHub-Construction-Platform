...
### 🧭 依存更新 4 軸モデル（blast radius 最小化戦略）

```mermaid
flowchart LR
    classDef critical fill:#fee,stroke:#c33,color:#900,stroke-width:2px
    classDef minor fill:#efe,stroke:#393,color:#060
    classDef major fill:#ffe,stroke:#990,color:#660
    classDef done fill:#eef,stroke:#33c,color:#006

    A[🔍 npm audit / pip-audit<br/>CVE / 脆弱性検知]:::critical --> B{重大度判定}
    B -->|CRITICAL / HIGH| C[🐍 backend patch<br/>即座に CVE 修正]:::critical
    B -->|patch / minor| D{言語スタック}
    B -->|major| E[🆕 backend major<br/>1 Issue / 1 PR]:::major

    D -->|🐍 Python| F[🐍 backend patch<br/>semver patch まとめ適用]:::minor
    D -->|⚛️ JS/TS caret| G[⚛️ frontend patch/minor<br/>npm update --save]:::minor
    D -->|⚛️ JS/TS exact-pin| H[⚛️ frontend exact-pin<br/>手動 1 件ずつ bump]:::minor

    C --> I[✅ CI 12-jobs PASS<br/>STABLE 判定]:::done
    F --> I
    G --> I
    H --> I
    E --> I

    I --> J[🔁 main へ admin-merge<br/>Deploy to Staging 自動発火]:::done
```
...
| [#218](https://github.com/Kensan196998G/ServiceHub-Construction-Platform/pull/218) | README v1.1.0 後の依存メンテナンス履歴セクション追加（本セクションを生成したメタ PR / docs-only / 12-jobs CI 全 PASS） |
...
| [#189](https://github.com/Kensan196998G/ServiceHub-Construction-Platform/issues/189) | メジャー更新調査（残: **starlette 1.x / redis 7.x / React 19 / vitest 4 / tailwind v4 / zod 4 / vite 8**）。openai 2.x は PR#193 / pytest-asyncio 1.x は PR#212 で消化済み。残 7 件は **互換性差分の調査中・分離スプリントで対応** |
